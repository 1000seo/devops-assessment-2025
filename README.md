...
- **선택 이유**
    ...
    - **ECS 기반 환경과 자연스럽게 연동됨**
      - ECS에 내장된 Blue/Green 기능을 사용하여 별도 CI/CD 도구 없이 TaskSet 기반 배포 가능

    - **서비스 중단 없이 배포 가능**
      - 신규 버전(Green)을 미리 띄운 뒤 Health Check가 통과될 때만 트래픽을 전환하므로 배포 중에도 서비스 중단이 없음

    - **롤백 속도가 매우 빠름**
      - 문제 발생 시 기존 Blue TaskSet으로 트래픽만 되돌리면 되므로 롤백 속도가 빠름
